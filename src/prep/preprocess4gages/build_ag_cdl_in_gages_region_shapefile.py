# --------------------------------
# Name:         build_ag_cdl_in_gages_region_shapefile.py
# Purpose:      Build agricultural land shapefiles from CDL rasters (only for agricultural land)
# --------------------------------
...
def main(overwrite_flag=False):
    ...
    cdl_ws = config.USDA.cdl_folder
    cdl_year = config.USDA.cdl_year
    cdl_format = config.USDA.cdl_format
    # It might make more sense to pass the non-ag CDL values instead
    cdl_crops = util.parse_int_set(config.USDA.cdl_crops)
