# --------------------------------
# Name:         et_demands_gages_region_stats.py
...
def cal_zonal_statistics(cfg_prep_used, overwrite_flag=False):
    ...
    for i in range(input_defn.GetFieldCount()):
        ...
        field_type_name = fieldDefn.GetFieldTypeName(fieldDefn.GetType())
        if field_type_name == 'Real' and fieldDefn.GetWidth() < 24 and fieldDefn.GetPrecision() > 0:
            copyDefn.SetWidth(24)
            copyDefn.SetPrecision(15)
        else:
            continue
        input_lyr.AlterFieldDefn(i, copyDefn, (ogr.ALTER_WIDTH_PRECISION_FLAG))
    # running this line when debugging will lead to an unexpected error
    # input_ds = None

    # Calculate lat/lon (central point lat/lon of a basin polygon)
    ...
    # use rtree to create the index of zone_extent to help us search thing quickly (the following "intersection")
    ...
    # zone_full_ds = None  # running this line when debugging will lead to an unexpected error
    ...
    for zone_fid, crop_fid_list in sorted(zone_crops.items()):
        ...
        if soil_crop_mask_flag or save_crop_mask_flag:
            ...
            zone_crop_poly = unary_union(zone_crop_polys).buffer(simplify_threshold).buffer(-simplify_threshold)
            ...
            if zone_crop_poly.is_empty:
                logging.debug('  ZONE FID: {} - empty polygon, skipping'.format(zone_fid))
                continue
    ...
    def zonal_stats(input_path, input_field, zone_wkt_dict, zone_rtree):
        ...
        # input_ds = None  # running this line when debugging will lead to an unexpected error
