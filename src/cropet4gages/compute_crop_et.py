...
def compute_crop_et(data, et_cell, crop, foo, foo_day, debug_flag=False):
    ...
    if debug_flag:
        logging.debug('compute_crop_et(): ppt_inf_prev %.6f  ppt_inf %.6f  SRO %.6f  precip %.6f' %
                      (foo.ppt_inf_prev, foo.ppt_inf, foo.sro, foo_day.precip))
        logging.debug('compute_crop_et(): depl_surface %.6f  wt_irr %.6f' % (foo.depl_surface, foo.wt_irr))
        logging.debug('compute_crop_et(): depl_ze %.6f  depl_zep %.6f' % (foo.depl_ze, foo.depl_zep))
    ...
    if debug_flag:
        logging.debug('compute_crop_et(): TEW %.6f  depl_ze %.6f  watin_ze %.6f' % (foo.tew, foo.depl_ze, watin_ze))
        logging.debug('compute_crop_et(): TEW %.6f  depl_zep %.6f  watin_zep %.6f' % (foo.tew, foo.depl_zep, watin_zep))
        logging.debug('compute_crop_et(): fw_irr %.6f stn_whc %.6f  AW %.6f  TEW %.6f' %
        ...
        logging.debug(
            'compute_crop_et(): watin_zep %.6f  fewp %.6f  totwatin_ze %.6f' % (watin_zep, fewp, foo.totwatin_ze))
    ...
    if debug_flag:
        logging.debug(
            'compute_crop_et(): TEW2 %.6f  ETref30 %.6f  etr_threshold %.6f' % (foo.tew2, foo.etref_30, etr_threshold))
        logging.debug('compute_crop_et(): tew2use %.6f  rew2use %.6f' % (tew2use, rew2use))
    ...
    if foo_day.snow_depth > 0.01:
        ...
        k_rad = (0.000000022 * foo_day.doy ** 3 - 0.0000242 * foo_day.doy ** 2 + 0.006 * foo_day.doy + 0.011)
        albedo_snow = 0.8
        albedo_soil = 0.25
    ...
    if debug_flag:
        logging.debug(
            'compute_crop_et(): kc_mult %.6f  ke %.6f  ke_irr %.6f  ke_ppt %.6f' % (kc_mult, ke, ke_irr, ke_ppt))
        logging.debug(
    ...
    if foo.depl_ze > foo.tew:
        ...
        if foo.depl_ze > foo.tew + 0.2:
            logging.warning(('Problem in keeping depl_ze water balance within TEW.' +
                             'depl_ze, TEW, e_irr, te_irr, e_factor = {} {} {} {} {}').format(depl_ze, tew, e_irr,
                                                                                              te_irr, e_factor))
            return
    ...
    if foo.depl_zep > foo.tew:
        ...
        if foo.depl_zep > foo.tew + 0.2:
            logging.warning(('Problem in keeping De water balance within TEW.  ' +
                             'De, TEW, E_irr, te_irr, e_factor = {} {} {} {} {}').format(depl_ze, tew, e_irr, te_irr,
                                                                                         e_factor))
            return
    ...
    if debug_flag:
        logging.debug('compute_crop_et(): depl_root %.6f  etc_act %.6f  ppt_inf %.6f' %
                      (foo.depl_root, foo.etc_act, foo.ppt_inf))
        logging.debug('compute_crop_et(): irr_real %.6f  irr_manual %.6f  irr_special %.6f' %
    ...
    if foo.irr_flag:
        ...
        if crop_doy < 1:
            crop_doy += 365
        if (crop_doy >= crop.days_after_planting_irrigation and foo_day.doy >= doy_to_start_irr and foo.in_season and
